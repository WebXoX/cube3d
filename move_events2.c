...
{  
	// while (vars->move.w == 1)
	// {
		if (vars->cx + vars->player.dx * 0.65   < vars->longest_row && vars->cx + vars->player.dx * 0.65   >= 0 && vars->cy < vars->final_c && vars->cy >= 0
        && vars->map[(int)vars->cy][(int)(vars->cx + vars->player.dx * 0.65 ) ] != 1)
			vars->cx  += vars->player.dx*(0.51 ) ;

		if (vars->cx  < vars->longest_row && (vars->cx) >= 0 
		&& (int)(vars->cy  + vars->player.dy * 0.65) < vars->final_c 
		&& (int)(vars->cy  + vars->player.dy * 0.65) >= 0
			&& vars->map[(int)(vars->cy  + vars->player.dy * 0.65)][(int)(vars->cx) ] != 1)
...
int move_a(t_data *vars)
{
    ...
    // {
        ...
        if (vars->cx +cos(rayAngle)*0.65  < vars->longest_row && vars->cx +cos(rayAngle)*0.65 >= 0 && vars->cy < vars->final_c && vars->cy >= 0
        && vars->map[(int)vars->cy][(int)(vars->cx +cos(rayAngle)*0.65) ] != 1)
        {
            vars->cx  += cos(rayAngle)*0.51 ;
        }
        if (vars->cx  < vars->longest_row && (vars->cx) >= 0
        && (int)(vars->cy +sin(rayAngle)*0.65) < vars->final_c
        && (int)(vars->cy + sin(rayAngle)*0.65) >= 0
            && vars->map[(int)(vars->cy + sin(rayAngle)*0.65)][(int)(vars->cx) ] != 1)
        {
...
int move_d(t_data *vars)
{
    ...
    // {
        double rayAngle = (atan2l(vars->player.dy,vars->player.dx) - vars->directionstart*(PI/2));
        if (vars->cx - cos(rayAngle)*0.65  < vars->longest_row && vars->cx - cos(rayAngle)*0.65 >= 0 && vars->cy -  sin(rayAngle)*0.65< vars->final_c && vars->cy -  sin(rayAngle)*0.65 >= 0
            && vars->map[(int)(vars->cy)][(int)(vars->cx - cos(rayAngle)*0.65) ] != 1)
        {
            vars->cx  -= cos(rayAngle)*0.51 ;
        }
